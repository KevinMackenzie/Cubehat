...
#include "../3rdParty/zlib-1.2.8/include/zlib.h"
...
#pragma pack()
	...
	bool CZipFile::Init( const std::wstring &resFileName )
	{
		...
		m_pDirData = QSE_NEW char[dh.dirSize + dh.nDirEntries*sizeof(*m_papDir)];
		if (!m_pDirData)
			return false;
	...
	bool CZipFile::ReadFile( int i, void *pBuf )
	{
		...
		char *pcData = QSE_NEW char[h.cSize];
		if (!pcData)
			return false;
	...
	bool CZipFile::ReadLargeFile( int i, void *pBuf, void( *progressCallback )(int, bool &) )
	{
		...
		if (err == Z_OK)
		{
			...
			while (stream.total_in < (uInt)h.cSize && !cancel)
			{
				...
				{
					QSE_ASSERT( 0 && "Something happened." );
					break;
				}
